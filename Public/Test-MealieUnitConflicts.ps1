...
<#
.SYNOPSIS
    Test for conflicts in unit JSON files
.DESCRIPTION
    Scans JSON files for naming conflicts that would cause import problems.
    Detects duplicate names, pluralNames, abbreviations, and aliases within and across files.
.NOTES
    Part of MealieSync module - see README.md for usage examples.
#>

function Test-MealieUnitConflicts {
    <#
    .SYNOPSIS
        Detect naming conflicts in unit JSON files
    .DESCRIPTION
        Scans unit JSON files for conflicts that would cause import problems:
    ...
    .PARAMETER Path
        ...
        Example: ".\Units\*.json" or @("VolumeUnits.json", "WeightUnits.json")
    .PARAMETER Folder
    ...
    .EXAMPLE
        Test-MealieUnitConflicts -Path ".\Units\*.json"
        # Check all JSON files in the Units folder
    .EXAMPLE
        Test-MealieUnitConflicts -Folder ".\UnitCategories" -Recurse
        # Check all JSON files in folder and subfolders
    .EXAMPLE
        $result = Test-MealieUnitConflicts -Path @("Volume.json", "Weight.json") -Quiet
        if ($result.HasConflicts) {
            Write-Error "Found $($result.ConflictCount) conflicts!"
        }
    .EXAMPLE
        # Check before import
        $check = Test-MealieUnitConflicts -Folder ".\Units" -Quiet
        if (-not $check.HasConflicts) {
            Get-ChildItem ".\Units\*.json" | ForEach-Object {
                Import-MealieUnits -Path $_.FullName -UpdateExisting
            }
        }
    .OUTPUTS
        [hashtable] @{
        ...
        @{
            ...
            Occurrences = @(
                @{ File = "filename"; Field = "name|pluralName|alias|abbreviation|pluralAbbreviation"; ItemName = "item name" }
            )
        }
    #>
